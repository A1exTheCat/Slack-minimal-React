...
const Channels = (props) => {
  ...
  /* три функции по клику, которые меняют стейт для модальных окон
    ...
    и отображаются по нажатию, внутрь передаем id канала с которым
    идет работаб так как это не обязательно активный канал */
  ...
  const channelsGenerator = (channel) => {
        ...
        <li className="nav-item w-100" key={channel.id}>
          <Dropdown as={ButtonGroup} className="d-flex">
            <Button
              ...
              onClick={() => dispatch(currentChannelIdActions.updateId(channel.id))}
            >
    ...
    /* логика дефолтных каналов, с проверкой текущего активного канала
      и действием на клик, меняющего текущий канал */
    return (
      <li className="nav-item w-100" key={channel.id}>
        <Button
          variant={channel.id === currentChannelId ? 'secondary' : 'light'}
