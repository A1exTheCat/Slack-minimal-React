...
const MainPage = () => {
  ...
  /* активируем useNavigate для переадресации на другую страницу,
    которые прописаны в роутере в файле App */
  const navigate = useNavigate();
  // активируем useDispatch для запуска экшенов для Стора
  const dispatch = useDispatch();
  /* оздаем реф для того тчо бы передать его с контекстом для компонентов,
    которые будут передавать данные и сокет в слайс */
  ...
  useEffect(() => {
    ...
    /* если прошла проверка сверху, то подтягивает стейт, при этом надо
      в каждый запрос добавлять токен авторизации в headers который
      в axios присоединяется в виде объекта */
    ...
    /* Подписываемся на обновления с сервера через socket.io c авторизацией,
      запускается только в первый рендер */
    const getSocket = () => {
      try {
        return io();
      } catch (e) {
        return dispatch(modalActions.changeToastMessage('forms.errors.networkError'));
      }
    };
    const socket = getSocket();
    /* записываем созданный сокет в ref созданный ранее для того тчо бы передать его
      с контекстом для компонентов, которые будут передавать
      данные и сокет в асинхронные экшены в слайсах */
    socketRef.current = socket;
    /* при отправке нами сообщения, или другими пользователями,
      мы не записываем сраазу в стор его, так как при отправке своих сообщений будет записываться
      дважды и понадобится дополнительная логика для обновления что бы этого избежать, мы записываем
      только когда получаем ответ от сервера, когда всё записалось там после\ нашего отправленного
      через сокет запроса на запись новых данных на сервер, из thunk в слайсах
      Открываем тут каналы для "прослушки" данных от сервера и вызовов
      соответствующих экшенов в слайсах для записи в стор */
  ...
  /* useEffect реагирующий на изменения toastMessage и вызывающий показ уведомления,
    прописываем условие, что бы не сраатывало при первом рендере */
  useEffect(() => {
    if (toastMessage) {
      notify(t(toastMessage));
    }
    dispatch(modalActions.resetToastMessage());
  }, [toastMessage, t, dispatch]);
      ...
      <div className="h-100" id="chat">
        <div className="d-flex flex-column h-100">
          {/* Передаем props в  NavbarComp c запущенной функцией авторизации из контекста
            которая возвращает true или false и меняет внешний
            вид NavBar с кнопкой Выход или без */}
          <NavbarComp authorization={isAutorization()} />
          <Container className="h-100 my-4 overflow-hidden rounded shadow">
            <div className="row h-100 bg-white flex-md-row">
              {/* Передаем в пропсы каналы и текущий активный ID канала, а в чат
                текущий канал и текущие сообщения, всё сделано так что бы при переключении
                каналов, сообщения сразу менялись на нужные */}
