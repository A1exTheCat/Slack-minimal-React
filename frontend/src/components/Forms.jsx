...
export const LoginForm = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        /* отправляем запрос на сервер(предварительно запускаем его если темтим с компа),
          отправляем наши данные на сервер, получем токен и записываем его в локал сторадж,
          оттуда мы его потом можем вытаскивать в любых местах в браузере */
...
export const ChatForm = () => {
  ...
  /* передаем сокент из контекста для отправки сообщений через thunk,
    сокет передает вторым параметром */
  ...
  /* в форме для блокировки кнопки отправки при пустом поле добавляем
    атрибут на кнопку disabled={!formik.isValid} */
  return (
      ...
      <InputGroup className="has-validation">
        ...
        <Button variant="text-primary" type="submit" className="btn-group-vertical border-0" disabled={!formik.isValid}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="20" height="20" fill="currentColor">
            <path
              fillRule="evenodd"
              d="M15 2a1 1 0 0 0-1-1H2a1 1 0 0 0-1
              1v12a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V2zM0 2a2 2 0 0 1 2-2h12a2
              2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V2zm4.5 5.5a.5.5 0 0 0 0 1h5.793l-2.147 2.146a.5.5 0 0 0 .708.708l3-3a.5.5 0 0 0 0-.708l-3-3a.5.5 0 1 0-.708.708L10.293 7.5H4.5z"
            />
          </svg>
...
export const RegistrationForm = () => {
    ...
    onSubmit: async (values) => {
      try {
        ...
        /* отправляем запрос на сервер, отправляем наши данные на сервер,
          получем токен и записываем его в локал сторадж, оттуда мы его
          потом можем вытаскивать в любых местах в браузере */
        ...
        /* так как вернулся токен, значит аутификация прошла удачно
          и мы переадресовываем на основную страницу чата */
        navigate('/');
      } catch (e) {
  ...
  /* небольшая функция для проверки было ли уже поле тронуто и имеет при этом
    уже ошибки, если хотя бы один false, то вернет false */
  ...
  /* В форме добавляем атрибуты onBlur для отслеживания касаний инпутов, иначе не зафиксируется
    и в классы добавляем функцию для проверки на касания и ошибки что бы предупреждение
    выводилось только для одного инпута и только если его уже коснулись и оно не прошло валидацию
    Следом идет блок с выводом ошибок, в нем проверка опять функцией на ошибки и касание,
    если функция возвращает тру, то рендерится ошибка из формика */
  return (
    <Form onSubmit={formik.handleSubmit} className="col-12 col-md-6 mt-3 mt-mb-0">
